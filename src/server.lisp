	 ...
	 (let ((virtual-folder "/weblocks-common/pub/")
	   ...
	   (return-from weblocks-dispatcher
	     (funcall (create-folder-dispatcher-and-handler virtual-folder physical-folder content-type)
		      request))))
	((and (weblocks-webapp-force-hostname app)
	      (not (equalp (car (cl-ppcre:split ":" (hunchentoot:host)))
			   (weblocks-webapp-force-hostname app)))
	      (webapp-serves-hostname (hunchentoot:host) app))
	 (hunchentoot:redirect (request-uri* request)
			       :host (weblocks-webapp-force-hostname app)
			       :port (let ((p (acceptor-port *weblocks-server*)))
				       (and (/= p (if (ssl-p) 443 80))
					    p))
